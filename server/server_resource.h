    ...
    const std::shared_ptr<http_response> render(const http_request& req) {
        ...
        if (req.get_digested_user() == user && req.check_digest_auth("MyRealm", pass, 300, &reload_nonce)) {
            if (req.get_arg("arg1") == "image") {
                return std::shared_ptr<file_response>(new file_response("mime_files/mob.png", 200, "image/png"));
            } else if (req.get_arg("arg1") == "xml") {
                return std::shared_ptr<file_response>(new file_response("mime_files/sample.xml", 200, "text/xml"));
            } else {
